...
ClusterNode::create_publisher(
  ...
  auto pub = rclcpp::create_publisher<
      MessageT,
      AllocatorT,
      ClusterNodePublisher<MessageT, AllocatorT>>(*this, topic_name, qos, options);
...
auto ClusterNode::declare_parameter(
  ...
  return this
      ->declare_parameter(name, value.get_type(), parameter_descriptor, ignore_override)
      .get<ParameterT>();
}
...
template <typename ParameterT>
bool ClusterNode::get_parameter(const std::string& name, ParameterT& parameter) const {
