...
int main(int argc, char** argv) {
  try {
    ...
    node->register_on_committed([&](int64_t id,
                                    akit::failover::foros::Command::SharedPtr command) {
    ...
    auto timer_ = rclcpp::create_timer(node, rclcpp::Clock::make_shared(), 2s, [&]() {
      node->commit_command(
          akit::failover::foros::Command::make_shared(std::initializer_list<uint8_t>{ch}
          ),
          [&](akit::failover::foros::CommandCommitResponseSharedFuture response_future
          ) {
